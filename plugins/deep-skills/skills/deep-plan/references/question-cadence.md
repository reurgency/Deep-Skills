...
| **3-at-a-time** | Medium features with a few independent decision areas. | Batch three related questions per round via `AskUserQuestion`. |
| **all-at-once** | Small, well-scoped features. **Default recommendation for small work.** | Render the full question set in as few `AskUserQuestion` calls as possible. |

Notes:
- `AskUserQuestion` supports up to 4 questions per call — for 3-at-a-time use one call; for all-at-once use as few calls as needed.
