...
The multi-agent run ends at the assembled report, exactly like the default flow — all findings `open`, nothing written to the plan. Triage and fix-phase routing are the separate `--triage` step (SKILL.md § 7); verified findings get no special handling there.
