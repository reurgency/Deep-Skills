...
description: Independently review implemented code with fresh, evidence-driven agents — correctness bugs, last-mile functional gaps (UI that looks wired but isn't), plan conformance, and coherence — then triage findings with the user and route accepted fixes into the plan for /deep-implement. Use after /deep-implement, before merge, or on any branch/PR/diff. Triggers on /deep-code-review and on requests to deep-review code, a PR, a branch, or a diff. Reviews code only — it never edits application source.
---

# DeepCodeReview

Independently review **implemented code** the way `/deep-review` reviews a *plan*: fresh agents, evidence-required findings, human-in-the-loop triage, and deferreds routed to the plan's **Deferreds** ledger. **Review only — never edit source** (see Guardrails for what "source" means).

This is the fourth skill of the `deep-*` series: `/deep-plan` (produce) → `/deep-review` (critique the plan) → `/deep-implement` (execute) → **`/deep-code-review` (critique the code)**. It closes the loop: the gate between "the implementation agent says it's done" and "this is actually done."
...
- `report.md` (← `templates/report.md`) — the human triage doc: verbatim scope line, pre-pass results, severity rollup, findings grouped by tier with lens tags and Status fields.
- `findings.json` (← `templates/finding.json`) — the machine record; numeric severity canonical.
- `certificate.md` (← `templates/certificate.md`) — pass/fail verdict (pass = no unresolved Blockers) + rollup + triage outcomes.

Update the effort manifest (created if absent — any deep-* skill owns manifest creation on first write).

Triage: **Blockers auto-route to the fix-phase — no triage, user informed; everything else goes to HITL triage**, finding by finding — **fix** (→ fix-phase) / **defer** (→ plan's Deferreds ledger) / **reject** (recorded with status "rejected by user").

### 6. Fix-phase routing
Accepted findings become a **fix-phase appended to the plan**, structured per deep-plan's phase conventions — Goal / Prerequisites / Files / Steps (one per accepted finding, citing its evidence) / Acceptance — so `/deep-implement` can execute it cold. In no-plan mode, generate a **minimal fix-phase plan stub** at `.deep-skills/<effort>/01-Plan/plan.md` (Context + the fix-phase only, same phase structure) so the implement loop stays available. Update the effort manifest's Code Review status on completion.
...
- **Review only — never edit source, where "source" = application code.** The skill MAY write `.deep-skills/` artifacts, which are controlled documents: `report.md`, `findings.json`, `certificate.md`, the effort manifest, fix-phase appends to the plan, and the no-plan-mode plan stub. Nothing else. If the user wants fixes built, point them to `/deep-implement`.
