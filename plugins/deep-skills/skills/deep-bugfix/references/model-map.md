...
## deep-bugfix's tier assignments

| Agent | Tier | Why |
|---|---|---|
| **Diagnose + fix** (one per cluster) | **main** | Root-cause diagnosis is the heaviest reasoning in the pipeline — a wrong cause poisons the fix, the proof, and the statuses. |
| **Adversarial proof** — cluster contains any defect at **severity ≥5** (or unset severity) | **main** | Refuting "fixed" on a real defect is the guarantee the skill sells; don't cheapen it where it matters. |
| **Adversarial proof** — all defects below severity 5 | **mid** | Nit-level chains are shorter; mid-tier reasoning suffices to trace them honestly. |
| **Containment / mechanical runs** (dependent discovery, test execution, result classification) | **cheap** | Mechanical work — the cheapest *reasoning* tier (much of it can be plain scripts needing no agent at all). |
...
All three tiers are **reasoning tiers**. `cheap` is the cheapest model that still does genuine **thinking** work — *not* a small/utility model. Every task in this series — diagnosing, proving, containing, synthesizing — is reasoning work; there is **no** utility-grade task (bulk labeling, slug or key generation, mechanical summarization) anywhere in the Deep Skills series that would justify a small model. Consequently the host's smallest models — **Haiku, GPT-mini, GPT-nano** — are **never required anywhere in this series** and never appear in this map.
...
1. **Pin the FULL model ID, never an alias.** Pass the exact IDs above at every launch. A bare alias like `opus` is resolved by the harness, not this table — a calibration run that launched mid-tier agents with the `opus` alias got Opus **4.6**, silently below the tier the table mandates.
...
4. **Small models never appear — any tier, any stage, any agent.** Haiku, GPT-mini, and GPT-nano are never assigned to diagnose, proof, containment, or synthesis (which is everything this skill does), on any host, at any ceiling. A calibration run in this series whose agents silently routed to Haiku produced 0 Blockers and 0 Majors on a diff with two known Criticals — output worse than none, because it ships false confidence.
