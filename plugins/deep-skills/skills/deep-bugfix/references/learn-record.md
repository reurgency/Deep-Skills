...
| `proof {tier, verdict}` | `tier` is the **run's proof tier** (`references/proof-of-fix.md`): `chain-trace` by default, `red-green` under `--reproduce` — except that a cluster where any defect was proven by the chain-trace fallback records `chain-trace`, the tier every member was actually proven at (the fallback itself stays visible as its reproduction-failed finding in the proof record and `fix-summary.md`). `verdict` comes from the **adversarial proof agent's** returned verdict shape — always `fixed` here by the emission rule, carried explicitly so the future reader never infers it. |
