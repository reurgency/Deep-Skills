...
- **Fix it with a reproducing test** (`--reproduce`) — upgrade proof from static chain-trace to a red→green test. **Lands in Phase 2** — if invoked today, say so and proceed with chain-trace proof.
- **Fix in a worktree** (`--worktree`) — isolate fix work in a git worktree. **Lands in Phase 3** — if invoked today, say so and proceed on the current branch (branch-first rules still apply).
